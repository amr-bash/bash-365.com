...
### Added
- **Kill switches and OAuth-first auth for the scheduled AI lanes** — the weekly content gardener, content review, and preacher workflows now idle on schedule until a human sets their repository variable (`CONTENT_GARDENER_ENABLED`, `CONTENT_REVIEW_ENABLED`, `PREACHER_ENABLED` — `gh variable set <NAME> --body true`), matching the content loop's `CONTENT_LOOP_ENABLED`, so a misbehaving loop is stopped by flipping a variable rather than editing a workflow; manual `workflow_dispatch` runs still proceed whenever a credential exists. All three now pass `ANTHROPIC_API_KEY` to `claude-code-action` only when `CLAUDE_CODE_OAUTH_TOKEN` is absent (the action does not reliably prefer OAuth when both inputs are given), so the subscription token is never undercut by API billing. Adds `.claude/skills/_shared/quarantine.md` — the shared agent guardrails (untrusted-input quarantine, honesty rule, merge discipline) from the agent-context kit v0.4.0, which agents cite in one line instead of restating — and `fleet.manifest.yml`, the repo's AI lanes, switches, and tokens in the shared `fleet/v1` vocabulary. `docs/automation.md` documents all of it.
...
- **zer0 stack alignment** — `zer0.json` now speaks the zer0-CMS console dialect in full: a `platform` block, required fields reconciled with `scripts/content_lint.py` (a dated post requires `title description author date lastmod categories tags preview`; a new `service` type requires `description`), publish/modified/preview-image markers, the lint's 155-character description limit, and the lint as the `cms.verifyCommand`. The root `Gemfile` adds the `zer0-image-generator` gem (`bundle exec jekyll preview-images`), and `.github/workflows/zer0-doctor.yml` calls zer0-CMS's report-only contract check weekly and on demand.
...
### Removed
- **`_plugins/preview_image_generator.rb`** — the vendored fork of the theme's preview plugin. Its Liquid filters and tags were rendered nowhere (not in this site, not in the theme); it loaded only on the Azure stack — the github-pages gem forces safe mode on the Pages and local dev builds — where it computed a missing-preview count no template read; and the `zer0-image-generator` gem's `--list-missing` covers that report. `zer0.json` is also excluded from the build now; it had been served at `/zer0.json`.
